...
| Last-Updated: Thu Aug 27 02:58:45 2015 (-0400)
|           By: Pushpendre Rastogi
|     Update #: 84
'''
from test import get_model_predictions_on_test_data
...
def create_y_hat_from_search(y_dist_test, x_left, x_right, model_func,
                             max_points_to_look_at=1000):
    ...
    num_train = y_true_train.shape[0]
    ...
    y_hat = numpy.empty((num_train, 1), dtype='int32')
    ...
    for idx in range(min(num_train, max_points_to_look_at)):
        print idx
        x_left = x_left_train[idx, 0]
        x_right = x_right_train[idx, 0]
        tmp_idx = 0
        ...
        path_prob_2 = model_func(
            (eligible_hop_idx, x_right_arr), retval=model_func_retval)
        path2_argmax = path_prob.argmax(axis=1)
    ...
    calculate_accuracy(y_hat[:max_points_to_look_at],
                       y_true_train[:max_points_to_look_at])
    exit(1)
    return y_hat


def main(model_file):
    y_dist, (x_left, x_right, y_true), model_func = get_model_predictions_on_test_data(
        model_file)
